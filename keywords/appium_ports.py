...
class AppiumDriverFactory:
    def __init__(self):
        ...
        self.emulators = {}    
        self.appium_urls = {
            "phone": "http://127.0.0.1:4724",  # Different port for phone
            "automotive": "http://127.0.0.1:4723"  # Default port for AAOS
        }
    ...
    def setup_driver(self, emulator_uid, emulator_type, app_package=None, app_activity=None):
        ...
        try:
            ...
            appium_url = self.appium_urls.get(emulator_type, "http://127.0.0.1:4723")
            print(f"🔗 Connexion à Appium pour {emulator_type} sur {appium_url}")
            driver = Remote(appium_url, options=options)
            time.sleep(5)
            return driver
